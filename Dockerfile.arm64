# =============================================================================
# Fenexity MicroOCPP Simulator - Multi-Platform Build (ARM64/AMD64)
# =============================================================================
# Platform wird durch docker buildx --platform gesteuert
FROM alpine:latest
...
# Note: CMAKE_POLICY_VERSION_MINIMUM=3.5 is needed for mbedtls compatibility with newer CMake
RUN if [ "$OCPP_VERSION" = "2.0.1" ]; then \
        echo "Building for OCPP 2.0.1"; \
        cmake -S . -B ./build -DMO_ENABLE_V201=1 -DCMAKE_POLICY_VERSION_MINIMUM=3.5; \
    else \
        echo "Building for OCPP 1.6"; \
        cmake -S . -B ./build -DMO_ENABLE_V201=0 -DCMAKE_POLICY_VERSION_MINIMUM=3.5; \
    fi
...
RUN echo '#!/bin/bash' > /start.sh && \
    ...
    echo '    echo "[FENEXITY_STARTUP] ✅ WebSocket configuration found"' >> /start.sh && \
    echo '    break' >> /start.sh && \
    echo '  fi' >> /start.sh && \
